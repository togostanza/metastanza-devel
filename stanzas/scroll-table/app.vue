  ...
  >
    <div class="tableWrapper" @scroll="handleScroll">
      <table v-if="state.allRows">
        ...
        <tbody>
          <tr
            v-for="(row, row_index) in state.allRows"
            :key="row.id"
      ...
      <div
        v-else-if="
          state.allRows && state.allRows.length === 0 && !state.isFetching
        "
...
export default defineComponent({
  ...
  setup(params) {
          ...
          data.map((row) => {
            return state.columns.map((column) => {
              return {
                column,
                value:
                  column.id === METASTANZA_NODE_ID_KEY
    ...
    const isSelectedRow = (row) => {
      ...
      return state.selectedRows.includes(String(rowID));
    };
