...
import { f as format, l as linear } from './linear-1b2cc043.js';
import { b as band } from './band-98510399.js';
import { l as log } from './log-b4392039.js';
import { a as axisBottom, c as axisTop, d as axisRight, b as axisLeft } from './axis-3dba94d9.js';
...
const timeIntervalUnitsSchema = mod
    .union([
    mod.literal("second"),
    mod.literal("minute"),
    mod.literal("hour"),
...
const paramsModel = mod
    .object({
    "axis-x-ticks_label_angle": mod.number().default(0),
    "axis-y-ticks_label_angle": mod.number().default(0),
    "axis-x-title_padding": mod.number().default(0),
    "axis-y-title_padding": mod.number().default(0),
    "axis-x-title": mod.string(),
    "axis-y-title": mod.string(),
    "axis-x-placement": mod
        .union([mod.literal("top"), mod.literal("bottom")])
        .default("bottom"),
    "axis-y-placement": mod
        .union([mod.literal("left"), mod.literal("right")])
        .default("left"),
    "axis-x-scale": scaleSchema,
    "axis-y-scale": scaleSchema,
    "axis-x-gridlines_interval": mod.number().optional(),
    "axis-x-gridlines_interval_units": timeIntervalUnitsSchema,
    "axis-y-gridlines_interval": mod.number().optional(),
    "axis-y-gridlines_interval_units": timeIntervalUnitsSchema,
    "axis-x-ticks_interval": mod.number().optional(),
    "axis-x-ticks_interval_units": timeIntervalUnitsSchema,
    "axis-y-ticks_interval": mod.number().optional(),
    "axis-y-ticks_interval_units": timeIntervalUnitsSchema,
    "axis-x-ticks_labels_format": mod.string().optional(),
    "axis-y-ticks_labels_format": mod.string().optional(),
    "axis-x-range_min": mod.number().optional(),
    "axis-y-range_min": mod.number().optional(),
    "axis-x-range_max": mod.number().optional(),
    "axis-y-range_max": mod.number().optional(),
})
    .passthrough();
var AxisScaleE;
(function (AxisScaleE) {
    AxisScaleE["linear"] = "linear";
    AxisScaleE["log10"] = "log10";
    AxisScaleE["time"] = "time";
    AxisScaleE["ordinal"] = "ordinal";
})(AxisScaleE || (AxisScaleE = {}));
var AxisPlacementE;
(function (AxisPlacementE) {
    AxisPlacementE["left"] = "left";
    AxisPlacementE["right"] = "right";
    AxisPlacementE["top"] = "top";
    AxisPlacementE["bottom"] = "bottom";
    AxisPlacementE["default"] = "";
})(AxisPlacementE || (AxisPlacementE = {}));
...
const proxyfy = (init, callbackMap) => {
            ...
            if (key === "params") {
                ...
                Object.entries(val)
                    ...
                    if (newType !== "object") {
                        ...
                        const isChanged = (pValue.length !== oldParams[pKey].length ||
                        ...
                        const ifChanged = Object.entries(pValue).some(([subKey, subVal]) => {
...
class Axis {
    ...
    _axisMargin = { LEFT: 0, TOP: 0, BOTTOM: 0, RIGHT: 0 };
    ...
    constructor(svg) {
        ...
        this.callbackMap.set("titlePadding", this._handleTitlePaddingUpdate.bind(this));
        this.callbackMap.set("margins", this._handleMarginsUpdate.bind(this));
        this.callbackMap.set("tickLabelsAngle", this._handleTickLabelsAngleUpdate.bind(this));
        this.callbackMap.set("scale", this._handleScaleUpdate.bind(this));
        this.callbackMap.set("gridInterval", this._handleGridIntervalUpdate.bind(this));
        this.callbackMap.set("ticksInterval", this._handleTicksIntervalUpdate.bind(this));
        this.callbackMap.set("ticksLabelsFormat", this._handleTicksLabelsFormatUpdate.bind(this));
        this.callbackMap.set("drawArea", this._handleDrawAreaUpdate.bind(this));
        return proxyfy(this, this.callbackMap);
    }
    _init() {
     ...
     */
    _getTickValues(interval, ticksOrGrid) {
        if (interval) {
            if (this.params.scale === "linear" || this.params.scale === "log10") {
                const domain = this.params.domain;
                ...
                switch (ticksOrGrid) {
                    case "ticks":
                        return intervalMap[this.params.ticksIntervalUnits]().every(interval);
                    case "grid":
     ...
     */
    get gridTickSize() {
        if (this.params.placement === AxisPlacementE.bottom ||
    ...
    _handleDrawAreaUpdate(newDrawArea) {
        this._width = newDrawArea.width;
        this._height = newDrawArea.height;
        this._titleG.attr("transform", getTitleTranslate.call(this, this.params.placement));
    }
    _handleGridIntervalUpdate() {
        this._updateGrid();
    ...
    _callDrawAxis() {
        ...
        this._axisG.selectAll(".tick").each(function () {
    ...
    _handleMarginsUpdate() {
        this._calcAxisMargins();
        this._callDrawAxis();
        this._callDrawGridlines();
        this._g.attr("transform", `translate(${this._axisMargin.LEFT}, ${this._axisMargin.TOP})`);
        this._titleG.attr("transform", getTitleTranslate.call(this, this.params.placement));
    }
    _handleDomainUpdate(domain = this.params.domain) {
    ...
    _handlePlacementUpdate() {
        ...
        this._redrawAxis();
        this._titleG.attr("transform", getTitleTranslate.call(this, this.params.placement));
        this._titleText.attr("alignment-baseline", getTitleBaseline(this.params.placement));
        if (this.params.placement === "left" || this.params.placement === "right") {
            this._titleText.attr("transform", "rotate(-90)");
        }
    }
    _handleTitleUpdate(title) {
        this._titleText.text(title);
    }
    _handleTitlePaddingUpdate(padding = this.params.titlePadding || 0) {
        let translate;
        const previousTransform = this._titleG.attr("transform");
        const prevTranslate = /translate\((-?\d+),\s*(-?\d+)/.exec(previousTransform);
    ...
    _handleTickLabelsAngleUpdate(angle = this.params.tickLabelsAngle || 0) {
        let translate = "";
    ...
    _checkScaleErrors(scale, domain) {
        if (scale === "log10" && domain.some((d) => d < 0)) {
            throw new Error(`Log scale can be set only for domains with positive values. Got domain [${domain.join(",")}]`);
        }
        if (scale !== "ordinal" && domain.some((d) => typeof d === "string")) {
            throw new Error(`Scales other than ordinal cannot accept domain with strings. Got scale ${scale} with domain [${domain.join(",")}]`);
        }
    }
    _handleScaleUpdate(newScale) {
        ...
        this._axisGen = getAxisGen(this.params.placement)(this._axisScale);
        this._axisGen.tickFormat(this.ticksLabelsFormatter);
        this._gridGen = getAxisGen(this.params.placement)(this._axisScale);
        this._updateTicks();
        this._updateGrid();
        this._callDrawAxis();
        this._callDrawGridlines();
    }
    _handleTicksLabelsFormatUpdate() {
        this._axisGen.tickFormat(this.ticksLabelsFormatter);
        this._callDrawAxis();
    }
    _redrawAxis() {
    ...
    _updateTicks() {
        switch (this.params.ticksInterval) {
            ...
            default:
                if (this.params.scale === "time") {
                    if (this.params.ticksIntervalUnits &&
...
function getTitleBaseline(placement) {
    switch (placement) {
        case "top":
            return "baseline";
        case "bottom":
...
function getTitleTranslate(placement) {
...
export { Axis as A, paramsModel as p };
//# sourceMappingURL=AxisMixin-05cdb891.js.map
