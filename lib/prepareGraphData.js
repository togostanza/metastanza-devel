...
export default function (nodesC, edgesC, params) {
  ...
  if (
    nodesC.some(
      (d) => d[nodeColorParams.colorKey] || d[nodeColorParams.groupKey]
    )
  ) {
    const nodeColorFunc = color().domain(
      [...new Set(nodesC.map((d) => "" + d[nodeColorParams.groupKey]))].sort()
    );
    ...
    nodesC.forEach((node) => {
      ...
      if (regex.test(node[nodeColorParams.colorKey])) {
        node[symbols.nodeColorSym] =
          node[nodeColorParams.colorKey].toUpperCase();
      } else if (typeof node[nodeColorParams.groupKey] !== "undefined") {
        node[symbols.nodeColorSym] = nodeColorFunc(
          "" + node[nodeColorParams.groupKey]
        );
      } else {
        node[symbols.nodeColorSym] = "currentColor";
      }
    });
  } else {
    nodesC.forEach((node) => {
