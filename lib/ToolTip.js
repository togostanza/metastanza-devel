  ...
  render() {
    return html`
      ...
      <div class="tooltip ${this.showing ? "-show" : ""}" ${ref(this.tooltip)}>
        <div class="inner" .innerHTML=${this.tooltipContent}></div>
      </div>
    `;
  }
  ...
  getTemplateVariables(templateStr) {
    ...
    function walk(node) {
      ...
      if (
        ...
        // For BlockStatement, extract variables from params (the condition)
        if (
          node.type === "BlockStatement" &&
          node.params &&
          ...
          node.params.forEach((param) => {
            // param is a PathExpression, so parts is directly on param
            if (
              param &&
              param.type === "PathExpression" &&
              Array.isArray(param.parts)
            ) {
              vars.add(param.parts[0]);
            }
          });
        }
      }
