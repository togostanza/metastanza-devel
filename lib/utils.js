...
export function emitSelectedEvent({
  ...
  // get filter nodes
  const targetElements = stanza.selectAll(targetElementSelector);
  ...
  // the element can be passed as an argument or taken from the stanza object
  element = element === undefined ? stanza.element : element;
...
export function updateSelectedElementClassName({
  ...
  targetElements.classed(selectedElementClassName, (d) => {
    ...
    // convert targetId to string if it is a number
