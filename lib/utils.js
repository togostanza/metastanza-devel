...
export function selectElement({
  ...
  const targetElements = drawing.shadowRoot.querySelectorAll(
    targetElementSelector
  );
  for (const el of targetElements) {
    const isSelected = selectedIds.includes(el.id.replace(idPrefix, ""));
    const inputElement = drawing.shadowRoot.querySelector(`#${el.id}`);
