...
export default class Legend2 extends LitElement {
  ...
  constructor() {
    ...
    this.options = {};
    this.title = "";
    this._classes = { "marker-round": false, "marker-square": true };
    ...
    this.options.shape = this.options.shape ?? "round";
  }
  ...
  static get styles() {
    return css`
      ...
      .legend > table > tbody > tr > td > .marker-round {
        display: block;
        border-radius: 100%;
        margin: 0 auto;
      ...
      .legend > table > tbody > tr > td > .marker-square {
        display: block;
        margin: 0 auto;
        border: calc(var(--togostanza-border-width) * 1px) solid
          var(--togostanza-border-color);
      ...
      .legend > .title {
        font-size: var(--togostanza-fonts-font_size_small);
      }
      .leader {
        position: absolute;
  ...
  render() {
    return html`
      ...
      <div class="legend">
        ...
        <table
          style="${this.options.shape === "square"
            ? "border-spacing: 0;"
            : null}"
        >
          <tbody
                ...
                <td class="${item.toggled ? "toggled" : ""}">
                  <span
                    ...
                    style="background-color: ${item.color}; width: ${item.size
                      ? item.size + "px"
                      : "1em"};  height: ${this.options.shape !== "square"
                      ? item.size
                        ? item.size + "px"
                        : "1em"
                      : "100%"}"
    ...
    if (this.items.length) {
      ...
      if (this._highlightNodeMap.has(nodeId)) {
        ...
        if (this.options.showLeaders) {
          ...
          const targetRect = nodesToHighlight[0].getBoundingClientRect();
  ...
  _mouseOutHandler(e) {
    e.stopPropagation();
    if (this.items.length) {
      const nodes = this.items.flatMap((item) => item.nodes);
    ...
    if (changed.has("options")) {
      if (this.options.shape) {
        switch (this.options.shape) {
          ...
          default:
            this._classes = { "marker-round": true, "marker-square": false };
        }
      }
    }
  }
